...
import { Review } from '../../types'; // Adjust the import path as needed
...
const ReviewBody: React.FC<ReviewBodyProps> = ({ review, isDarkMode }) => {
  // Assuming 'intro' is a valid property to use instead of 'body'
  return (
    <div className={`prose mb-4 break-words max-w-full mt-8 ${isDarkMode ? 'prose-dark' : 'prose-light'}`}>
      {parse(review.intro)} {/* Adjust according to available properties */}
